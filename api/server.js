...
// When true, /sow-generator/api/extract returns canned sample data instead
// of calling the Anthropic API - lets the rest of the flow (review form,
// docx generation, download, My SOWs) be exercised with zero API cost
// while testing a fresh deploy, before ANTHROPIC_API_KEY is wired in for
// real. Never affects /generate - that step never calls the model anyway.
const SOW_MOCK_EXTRACTION = process.env.SOW_MOCK_EXTRACTION === "true";
...
// Deliberately mixes real content and "TBC" fields, same as a genuine
// extraction would when the transcript only covers some of the schema -
// gives a realistic feel for the review-form step, not just a happy-path
// fully-populated example.
var MOCK_SOW_DATA = {
  documentInfo: {
    proposalTitle: "Sample Network Refresh Proposal", clientName: "Sample Client Ltd",
    proposalReference: "TBC", projectName: "Sample Core Network Refresh", documentAuthor: "TBC",
    contactName: "TBC", contactPhone: "TBC", contactEmail: "TBC",
  },
  executiveSummary: {
    backgroundAndContext: "This is placeholder content generated by mock mode (SOW_MOCK_EXTRACTION=true), so the review/generate flow can be tested without calling the Anthropic API.",
    nextSteps: "TBC",
  },
  currentEnvironment: { overview: "TBC", currentServicesOverview: "TBC", requirementsSummary: "Sample requirement: replace ageing core switching estate." },
  solutionSummary: { overview: "Sample solution overview text.", components: "TBC" },
  rocServices: {
    engagementApproach: "Sample engagement approach text.", serviceTransition: "TBC",
    pricingBasis: "milestone",
    milestones: [
      { name: "Order Acceptance", completionDate: "TBD", percentCharge: "30%" },
      { name: "Go Live", completionDate: "TBD", percentCharge: "70%" },
    ],
    customerAssumptions: ["Sample assumption: customer provides site access within 5 working days of request."],
  },
  serviceOverview: { include: false, serviceQuality: "", itilServices: "", technologyManagement: "" },
  commercialSummary: {
    quoteReference: "TBC",
    serviceLineItems: [{ label: "Order Acceptance", rate: 5000, quantity: 1, total: 5000 }, { label: "Go Live", rate: 11667, quantity: 1, total: 11667 }],
    managedServiceUplift: null,
    azureCostEstimate: [],
    ongoingAnnualCosts: [],
  },
};
...
app.post("/sow-generator/api/extract", function (req, res) {
  ...
  if (SOW_MOCK_EXTRACTION) {
    return res.json({ data: MOCK_SOW_DATA, mock: true });
  }
